...
template<typename T>
void mergeSortAdvance(T arr[], int n) {
    assert(n);
    _mergeSortAdvance(arr, 0, n - 1);
}

//递归使用归并排序,排序[rangeL,rangeR]范围内的元素,注意左右都是闭区间
template<typename T>
void _mergeSortAdvance(T arr[], int rangeL, int rangeR) {
    // 优化2: 对于小规模数组, 使用插入排序
    if (rangeR - rangeL <= 16) {
    ...
    _mergeSortAdvance(arr, rangeL, center);
    _mergeSortAdvance(arr, center + 1, rangeR);

    // 优化1: 对于arr[center] <= arr[center + 1]的情况,不进行merge
    // 对于近乎有序的数组非常有效,但是对于一般情况,有一定的性能损失
    if (arr[center] > arr[center + 1]) {
        _mergeAdvance(arr, rangeL, center, rangeR);
    }
}

//归并[rangeL,center]和[center+1,rangeR]两段
template<typename T>
void _mergeAdvance(T arr[], int rangeL, int center, int rangeR) {
